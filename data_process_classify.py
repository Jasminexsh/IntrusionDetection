...
def pre_handel_data1():
    source_file = Train_File_classify
    handled_file = r'data\train_data_10_percent_corrected_classify.csv'
    ...
    with open(source_file, 'r') as data_source:
        ...
        for row in csv_reader:
            ...
            temp_line[41] = handle_label(row)  # 将源文件行中23种攻击类型转换成数字标识
...
def pre_handel_data2():
    source_file = Unlabeled_Test_File_classify
    handled_file = r'data\test_data_10_percent_corrected_classify_unlabeled.csv'
    data_file = open(handled_file, 'w', newline='')  # python3.x中添加newline=''这一参数使写入的文件没有多余的空行
    with open(source_file, 'r') as data_source:
        csv_reader = csv.reader(data_source)
        csv_writer = csv.writer(data_file)
...
def pre_handel_data3():
    source_file = Labeled_Test_File_classify
    handled_file = r'data\test_data_10_percent_corrected_classify_labeled.csv'
    data_file = open(handled_file, 'w', newline='')  # python3.x中添加newline=''这一参数使写入的文件没有多余的空行
    with open(source_file, 'r') as data_source:
        csv_reader = csv.reader(data_source)
        csv_writer = csv.writer(data_file)
...
def handle_protocol(inputs):
    ...
    if inputs[1] in protocol_list:
        return find_index1(inputs[1], protocol_list)[0]
